...
export function computeObjectiveStats(
  ...
  for (const t of trials) {
    const raw = t.results[key];
    if (raw === undefined) continue;
    const v = Number(raw);
    if (!Number.isFinite(v)) continue;
    entries.push({ value: v, trialKey: `${t.studyName}\0${t.trialId}`, complete: t.complete });
  }
  ...
  if (values.length === 0) {
    return { best: NaN, median: NaN, worst: NaN, count: 0, incompleteCount: 0, histogram: [], binTrialKeys: [], bestTrialKey: "" };
  }
  ...
  const worst = direction === "maximize" ? values[0]! : values[values.length - 1]!;
  ...
  return { best, median, worst, count: values.length, incompleteCount, histogram, binTrialKeys, bestTrialKey };
}
